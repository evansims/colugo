...
== Installation ==
	...
	1. Create a new directory on your web server via FTP. CHMOD that directory with
	   appropriate write permissions (0755 usually works.)
	...
	3. Assign the configuration values at the top of the script.
	   ...
	   $localBaseURL
	   Should point to the absolute URL where your images are being stored.
	   i.e., If your u.php script is in /i, it might be http://yourdomain.com/i/.
	   Be sure to include the last slash.
		
	   $twitterUsername
	   $twitterPassword
	   Your Twitter username and password. The script does not use these, as it does
	   not communicate with Twitter directly; however, Colugo checks this information against
	   the incoming image provided by Tweetie to ensure that you are indeed the one posting
	   the image. Security mechanism.
		
	   $licenseText
	   If you'd like license information to be written onto the image, provide it here.
		
	   $lessnEndpoint
	   Optional. If you have Shaun Inman's Lessn installed, provide the URL path to the
	   admin interface, i.e. http://yourdomain.com/g/-/.
	   Be sure to include the last slash.
		
	   $lessnAPI
	   Optional. If you have Shaun Inman's Lessn installed, provide your API key. The
	   API key can be found after you've logged into your admin interface, at the bottom
	   of the page (it's in light grey, so you might not notice it  at first.)
	
	4. Upload your modified u.php and the remainder of the contents of the archive
	   to the directory you created.

== Configure Tweetie ==

	1. Go to Tweetie's accounts listing interface, and tap "Settings" in the lower,
	   left hand corner.
	
	2. Tap Image Service,  and select Custom.
	
	3. Change the URL to point to where you uploaded the "u.php" file; i.e.
	   http://yourdomain.com/i/u.php.
